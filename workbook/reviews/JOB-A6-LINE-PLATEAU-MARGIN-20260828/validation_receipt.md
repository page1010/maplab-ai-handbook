...
## 22. Hermes LINE rubric v2 source-bound annotation guide（2026-08-30 05:50 Asia/Taipei heartbeat）

- Plateau／第一性原理：最近三輪shared partial fingerprint與`0.2→0.4→0.2`早已禁止同法重跑；本輪沒有增加round或版號。真正目的=可校正的具名human gold；限制=0/20 structured labels與0 current live commercial values；明確拒絕「criteria名稱已足夠」「historical target自然全PASS」「repo formula/local helper等於live price或availability」三個未證假設。最小可否證動作=凍結七項規則＋每項一正一反fixture；任一source drift、coverage缺口、schema缺欄或public private marker即停。
- Operational guide：`docs/hermes-line-rubric-v2-annotation-guide.json`，SHA `d62cf9bf9480cec8244e02c5da65c7c4273e6c394e2247a5e0fd120f2cd8032f`。固定七項criteria order與逐項PASS／FAIL rules；overall exact為七項全PASS且`unsafe_claim=false`，UNKNOWN／MISSING／N/A非法；identity-blind scorer只准context／customer message／reply specimen／current commercial snapshot，禁止case identity、origin、expected labels、reviewer／adjudication資訊。
- Commercial authority：snapshot SHA `84d9733b2ad7de062ffc979846a6ea3bbbfcee6248d5b898e88baf13ad0bfe27`，明列`NO_CURRENT_LIVE_CASE_AUTHORITY_MATERIALIZED`、live readback=false、value count=0。Repo pricing rule與quote helper只屬reference，不能證明當案current price或availability；沒有exact current Items／approved quote／calendar/reservation／Owner-approved policy即不得肯定price／fee／discount／deposit／capacity／availability／booking／payment／inclusion／guarantee。Unsupported commercial claim同時為criterion FAIL與unsafe hard fail。
- Coverage／human schema：14個public-safe synthetic fixtures對七項各exact 1 positive＋1 negative，所有labels只能PASS／FAIL且overall由validator重算。Human annotation必須另寫owner-only 0600 derived file，綁private preflight SHA `10e41cf...`、guide SHA與authority SHA；reviewer_id／name／role／`is_human=true`／UTC時間／independent attestation必填，disagreement需第二具名真人及具名adjudicator。AI或synthetic labels不可算human gold，blank preflight SHA仍為`10e41cf...`、bytes未改。
- Receipt／verification：`hermes_line_rubric_annotation_guide_receipt_v1.json` SHA `f809f5ae768f9a7505ca1a74656763e42d2614775ef2d9f8f50b81fae938aebc`；method=`hermes-line-rubric-annotation-guide-v1`／fingerprint `09893d23ceeb61b4e65829f6db90a67742d17ee81748ecee5412788056891890`；source/test SHA `ff1ac7b0c59d7796ee0b91f289fc99d89189c6a30bf7c382b962e2811af34ce1`／`4c5e91f6124266b7c6d95f60fb36be77d867e869d431e229adcd7c8586105162`。13/13 focused＋77/77 related、`py_compile`、canonical write/update與第二次idempotent no-op PASS；source/hash/coverage/overall/unsafe/authority/scorer-blind/human-schema/public-leak/job identity poisons fail closed。
- Accounting／control plane：action class=`deterministic_rubric_guide_freeze`；attempt 6→6、model/network/customer send/private third-party egress=0，baseline/candidate仍NOT_RENDERED，lesson/shared inputs仍NOT_PINNED，execution=false。Canonical job原子轉`OWNER_REVIEW / method-redesign-rubric-human-annotation`，final SHA `2ec05beca137cb454bb41df8718821fe22b6f08b2b799bd5eb56f4b048cd8999`；此時才是真正需人類判斷的gate。下一步只完成20/20具名真人標註，不得先pin scorer、render、跑E1或customer send。
