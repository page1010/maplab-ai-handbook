...
1. 嘗試 1-2 次自行修復
2. 2. 修不好 → 查 `skills/troubleshooting-hub.md`（症狀 → 解法 → 技能書）
   3. 3. 找到解法 → 照技能書做 → 繼續執行
      4. 4. 找不到解法 → 用 hub 內的回報格式記錄問題 → 回報 A1 或 owner
         5. 5. A1 補充到 hub → 下次所有 Agent 都能查到
           
            6. > 重點：troubleshooting-hub.md 是你的急救手冊，不是額外負擔。
               >
               > ---
## 完成任務後的收尾 SOP
