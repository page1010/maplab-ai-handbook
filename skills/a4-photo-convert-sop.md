...
5. **轉檔≠放行，但閘門是自動的**（Owner msg 4780，2026-09-05 裁決：辨識不走人眼確認，排程自動完成）：webp 出來後走「雙模型自動確認閘」——兩個視覺模型獨立辨識，一致即定案；不一致走保守自動規則（alt 用中性事實寫法、疑似他牌 logo 直接不用該張），全程不等人。人眼只剩事後抽查，不是流程節點。
...
## v1.1 目標：圖形辨識雙模型互核（Owner msg 4771，2026-09-04）

背景：單一視覺模型的辨識／ALT 描述容易失準，交給 hermes 免費鏈同樣有此風險。
Owner msg 4780 追加裁決：**不走人眼確認，排程自動完成**。
設計（全自動閉環）：
1. 雙模互核：同一張圖由兩個「不同家」視覺模型獨立產出結果（ALT 候選、他牌 logo
   入鏡判定），程式比對。一致→定案。
2. 不一致→保守自動規則定案，不等人：ALT 退回中性事實寫法（場景+品項型態，不寫
   不確定的專名）；logo 判定任一模型說有→該張直接不用。人眼只做事後抽查。
3. 排程：辨識佇列（data/photo_convert/<slug>/recognize_queue.json）掛既有每小時
   排程輪詢；模型走 hermes 免費鏈視覺上游，403 滿額就本輪跳過、下輪重試，跨日
   額度重置自然補完——慢可以，停不行。
4. manifest 欄位：alt_model_a、alt_model_b、agree、final_alt、resolved_by
   （auto-consensus / auto-conservative；不再有 human 節點）。
落地順序：先跑通轉檔（無模型需求）→ 佇列檔生成 → hermes 視覺調用腳本 → 掛排程。

## Changelog

- v1.0（2026-09-04）：初版，依 msg 4751 建制；含缺陷1/2 根治設計。
- v1.0-verified（2026-09-05）：首發實跑通過——木地板兩夾去重 17 張全成功，manifest 一比一。根路徑修正：實際案夾在 GoogleDrive-pagewu1010 帳號 2026maplab外燴紀錄/ 下（原誤指 lb99104 MAPLAB_ASSETS）。
- v1.1 規劃（2026-09-05）：雙模互核（msg 4771）＋人眼閘拆除改全自動排程閉環（msg 4780）；待建 hermes 視覺調用腳本＋辨識佇列掛排程。
