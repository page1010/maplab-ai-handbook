...
## SECTION 11 — 新品對價、預算反推與忌口處理（Owner msg 2026-09-21 19:19 實作後回灌）

> 來源：Owner 要求「用預算反推、菜色以雷同的品項抓預算、抓完毛利、給我能出幾份的建議」。
> 實例：`outputs/a6-quote-20260921-seasia-100pax/QUOTE.md`（東南亞主題 100 人／30,000）。

### 11.1 售價與成本的唯一來源（別再找錯地方）

| 要什麼 | 去哪裡 | 不要去哪裡 |
|---|---|---|
| 售價 | `data/takeout_menu_prices.md`（外帶菜單售價表，報價 truth source） | Items 主表的 `default_price` 欄**全部是空的**，不要再翻 |
| 成本 | `data/items_master.json` 的 `default_cost`（只含食材） | 不要用售價回推成本 |

### 11.2 主表沒有的新品：雷同品項對價法（四步，不得跳過）

1. **售價**：在菜單裡找「同系列、同尺寸、同工序」的實價當基準，同系列有價差時按用料等級取位（有肉有辛香料取高標，純素取低標）。
2. **成本**：在 Items 主表找最接近的品項 `default_cost`，加減用料差異。
3. **全部標 `推估`**，並在報價單寫出「抓誰、為什麼」一行，讓 Owner 能一眼推翻。
4. **絕不自行發明價格**。菜單與主表都對不到（例如桶裝飲品沒有售價）→ 該項不報，寫「待 Owner 給價或授權倍率」。

### 11.3 預算反推法

1. 件數基準：人數 × 3～5 件。100 人／30,000 的實測落點是 **5 件／人、每人售價 300 元**。
2. 一律按**整組**配（8 片／12 入／15 入／10 吋），因為菜單規則是「每系列有最低訂購量、不同口味不可混搭」。
3. 給兩個方案：**滿預算版**（貼齊預算，尾數可折抵）＋**留餘裕版**（留 8%～10% 給飲品／車馬費／餐具）。
4. 毛利率只含食材，報價單必須明寫「不含人工、場佈、運送、耗材」，並提醒 100 人場若要現場服務／長桌／車馬費得另計。
5. 順手標出全單成本率最差的那一項，Owner 要壓毛利時直接從那裡下手。

### 11.4 忌口與過敏處理（人數要拆、醬料要問、宗教不猜）

1. 把每個忌口群組拆成人數 × 件數，算出該群組專屬品項要留幾份（例：蛋奶素 3 人 × 5 件＝15 件）。
2. 忌口品項**獨立擺盤加標牌**，避免夾取交叉污染。
3. 過敏群組：主動在選品階段就避開過敏原，不要事後補救（不選蘑菇鹹派、番茄系 pizza 等）。
4. **醬料成分一律問廚房，不替廚房斷言**（打拋醬、沙嗲醬常見含番茄糊；麵包是否含豬油；沙拉醬是否含魚露）。
5. **宗教禁忌不猜**：客人只說不吃牛，不等於可以吃豬。先問 Owner，不要自己假設。

### 11.5 菜單改版處理

Owner 傳新菜單照時，**不得直接覆蓋** `takeout_menu_prices.md`。做法是列出新舊差異表附在報價單或價目表底部，等 Owner 一句「以新圖為準」才整表改版。舊值劃掉保留可追溯。

### 11.6 A6 逾時升級

A6 沒回應時由 A0 直接代產報價單，報價單抬頭標明「代 a6 產出，a6 未回應」，並在回報裡照實說 a6 沒回，不幫它圓場。

---

*本技能書由 A6 在執行報價任務時讀取。品項組合邏輯基於 932 份歷史報價統計。*
*條款來源：data/quote-terms-reference.md | 成本來源：Items 主表 default_cost | 售價來源：data/takeout_menu_prices.md*

---

## SECTION 11 — 預算反推法與忌口配餐（Owner msg 2026-09-21T19:19，A0 代做後回寫）

> 觸發情境：客人**先給預算與人數**，菜色是口頭列的名字（可能不在主表）。
> 實例與完整算式見 `outputs/a6-quote-trial-20260921/QUOTE_100PAX_30K.md`（100 人 / 30,000）。

### 11.1 新菜色成本＝雷同品項代換（不准憑感覺填）

1. 先查主表有沒有同品項或同基底：例如「綠咖喱小鹹派」＝ APP043 泰式綠咖哩雞手工奶香小鹹派（25/個）就是完全命中；「梅子醬蝦棗」＝ APP038 台南古早味蝦棗（15/份）換醬，加醬料差額 3。
2. 主表沒有才代換，**一定要寫出抓誰、為什麼**：薄餅類對 APP022 / APP026（20/份）、加乳酪與肉的 pizza 片對 APP011（20/份）上浮到 25、三明治對 APP010（15/個）與 APP030（30/份）之間取 20。
3. 代換值標記為**估算不是實價**，開單前要 A1 或 Owner 覆核；覆核過才回寫 items_master，否則下一單又要重估。
4. 估算不確定性直接寫進風險段：新菜實作若超估值 20%，毛利率會掉幾個百分點要先講。

### 11.2 預算反推＝給三檔，不給單一數字

1. 件數基準：1 人 3 至 4 件。
2. 用一輪（每道各一件）的食材合計算平均單件成本，再乘件數。
3. 一定給三檔：精簡（3.0 件/人）、建議（3.5 至 3.6 件/人）、豐盛（4.4 件/人），每檔都列食材合計與系統毛利率，並明確推薦一檔。
4. 毛利率防線：**建議檔不低於 65%**，豐盛檔低於 62% 就要同步建議把報價往上調（例：30,000 的案子報 33,000 至 35,000）。
5. 反推完要回答「能出幾份」這句話本身：講總件數、每人件數、飲品桶數，不要只丟總價。

### 11.3 忌口與過敏配餐（有名單就要對得上人數）

1. 不吃牛：把主表所有牛品項（牛排普切塔 APP013、舒肥牛小排 APP016、溫式熟成牛排 MAIN003）排除，並在單上寫明「全場不上牛」。
2. 蛋奶素：清點六道裡他們能吃幾道；**不足 2 道就加素食備餐**，份數＝素食人數 × 4 件（建議 APP041 烤南瓜泥哈囉米起司羅勒卡納佩，無肉無菇無番茄）。
3. 過敏（番茄、蘑菇類）：能靠**改醬底**解掉就改（pizza 改沙嗲花生醬底或三杯醬油麻油底，全場就都能吃），不能改的品項直接不排（例：野菇薄餅）。
4. 交叉風險主動點出並回問，不自己決定：花生（沙嗲）、甲殼類（蝦棗）、是否要現場品項標示卡。
5. 紅線不變：**不判定飲食安全**，涉及過敏嚴重度一律標「需人工」由業務與客人確認。

### 11.4 每張反推單必附的兩段

1. **風險段**：系統毛利率只含食材（100 人場的人力、運送、餐具、場佈都不在內）、地點未定則車馬費待確認（出台南 3,000 至 4,000 會吃掉小案子 10% 以上）、兩週前預訂規則、報價數字永不自動發。
2. **對照驗算段**：用外帶含單價菜單反推每件零售價（目前基準：小漢堡 81/件、法式焦糖烤布丁 65/件），和本案每件報價比對，確認沒有低估自己。

### 11.5 a6 沒回的處理

a6 逾時未回應時，A0 直接用 items_master 代做，並在單上註明「a6 未回應，A0 代做」；**不替 a6 圓場、不假裝已派工**。代做完成後把方法回寫本 SECTION，讓下一次 a6 自己跑得出同樣結果。
