...
設計：同一張圖由兩個「不同」視覺模型各自獨立產出辨識結果（ALT 候選、logo 入鏡
判定），程式比對——一致即採用，不一致才送人眼確認閘。互核不取代人眼閘，只縮小
需要人看的量。啟用前置：至少兩個可用視覺後端（本機 ollama 視覺模型回歸、或
hermes 免費鏈中兩個不同家的視覺模型且額度可用）。實作時 manifest 加欄：
alt_model_a、alt_model_b、agree（yes/no）、final_alt、resolved_by（auto/human）。

## Changelog

- v1.0（2026-09-04）：初版，依 msg 4751 建制；含缺陷1/2 根治設計。
- v1.1 規劃（2026-09-05）：雙模型互核設計入檔（msg 4771）；執行等視覺後端到位。
