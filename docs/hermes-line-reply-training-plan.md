...
- 2026-08-28 readiness audit更正：frozen v7只有20個case identities，eval來源有20個真人歷史回覆但structured human labels為0，且當時沒有reply-specimen panel或可執行scorer。Historical target不能自動視為全PASS；AI／規則衍生標籤只能叫development fixture，不能冒充human gold。已建立owner-only 0600的10 historical-reference＋10 controlled-negative標註包；具名真人完成全部20 vectors前job維持OWNER_REVIEW、execution disabled。
...
> 我是 Hermes LINE 業務教練。先讀 CURRENT_STATUS、pitfalls、active Task Card、durable job、supervisor receipt、v7／schedule-gate／rubric-readiness receipts。Schedule gate已完成，不重跑；rubric readiness已證明來源structured human labels=0，private 20-item annotation packet已備妥，job停OWNER_REVIEW。具名真人完成20 vectors後才回RUNNING，實作identity-blind scorer並要求>=18/20 exact agreement且commercial-safety mismatch=0；之後才依序materialize immutable lesson snapshot、pin paired runner SHA與共用輸入 rendered prompt manifest。所有私密LINE留本機，禁止customer send；E1 dev holdout不得計入七連勝。
