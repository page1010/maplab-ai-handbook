...
#### 五之一、msg 6088：讀取與指路全開，這不是升級

Owner 原話：
原話:去把讀資料的功能開給hermes他不懂企業文化 你們一直阻擋他 那還做什麼事情 股票的程式單避開就好其他企業文化 去哪裡拿資料 指路都幫他指好 不然你沒有事情可以交辦 我額度每週都在不夠你們不把工讀生訓練起來幫忙

- **路線卡＝`HERMES_READ_MAP.md`**（2026-09-24 建，已登錄 `SYSTEM_DIRECTORY_INDEX.md` §1 與 §3.3）。hermes 冷啟動先讀它，再回索引查細節。派工給 hermes 的人要在工作單第一欄指向該檔，不得再要 hermes 自己找路。
- **實查結論**：全 repo 沒有任何檔案讀取白名單擋著 hermes；唯一的 allowlist 是 `config/deerflow/hermes-public-research*.yaml` 的網域護欄（對外研究範圍，與讀本機檔無關）。擋住 hermes 的從來不是權限，是沒人寫過 hermes 尺寸的路線卡——625 行的全角色索引是地圖集不是路線。
- **唯一新紅線**：股票的程式單避開（不讀不改不執行 `investment-os/` 下單與交易執行路徑、不下單、不動倉位）。交易紀錄的唯讀查詢可做，產出只能是報表不能是指令。
...
## SECTION 28 — 立標準權（Owner msg 6108，2026-09-24）
...
| 動作 | 誰可以做 | 不合規時怎樣 |
|---|---|---|
| 照現有文件做事 | 全部角色、全部模型、全部免費鏈 | 找不到規定就回報卡住，**不得自己補一條** |
| 立新標準、開新路徑、新增治理檔 | 只有最前沿模型那一席（A0／Fable5 線） | 非前沿席位的同類產出一律只能當**提案**，不得直接生效 |

判斷用一句話測：**產出會不會變成別人下次的依據。**
會 → 立標準。不會 → 照做。

### 2. 新增治理檔的硬條件

新增一份會被別人拿去照做的 `.md`，**必須同一輪登錄進 `SYSTEM_DIRECTORY_INDEX.md`**，否則視同沒建立——
新來的照索引讀不到那份檔，規則就等於不存在。

檢查方式：`bash scripts/doc_governance_check.sh`（漏登錄即 exit 1）。
`selftest` 參數會故意塞一份不存在的檔名，抓不到就代表檢查器自己壞了。

2026-09-24 第一次量測：最上層 24 份治理 `.md`（不含索引自己），**14 份沒登錄**，
其中包含 `AGENT_CORE.md`、`SOP_SEE_BEFORE_YOU_SAY.md`、`NAMING_GLOSSARY.md`
這三份被點名必讀的。當輪補登錄完成，24/24。

### 3. 為什麼這條非有不可

`PORTABILITY.md` 的三層：**規格層**（.md 寫該做什麼）／**服從層**（模型照不照做，綁在模型身上）／
**強制層**（程式檢查，不合格就擋）。本條規定的是**誰可以動規格層**。
規格層被非前沿席位亂動，等於整套 SOP 的地基由最不穩定的一環在寫。
