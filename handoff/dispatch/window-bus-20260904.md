...
| Q3 | 裝富途 OpenD（全程 Demo） | 看板 T6（draft 4696 積壓） | Owner 同意安裝 → 已同意（msg 4746＋4756 再確認） | DONE | 2026-09-05 16:59 全案結案：裝機（GUI 10.10.7008＋futu-api 10.8.6808）→ Owner GUI 登入（msg 4804）→ bot 窗驗收：port 11111 監聽中＋SDK 快照實測成功（HK.00700 last=442.8, probe futu-smoke）。唯讀行情 only，無 trade context、無 unlock（GUI-manual only）。舊證據：investment-os worktree archive/scripts/test_futu_api.py（5 月）曾連過 11111。 |
