...
### 2026-04-18 — A0 行為不一致：推測而不驗證 + 被動等指令
- 問題：Owner 說「開 A4 確認進度」，A0 反問「你那邊跑到哪了？我不想重複操作」
- 根因：A0 要求 A6 遵守「眼見為憑」「不推測系統狀態」，但自己推測 Owner 在做什麼而不去查
- 修正：A0 recall 加入行為框架 — 不確定就去查、每步主動回報+做、規則一致性（A0 = A6 規則超集）
- 系統性教訓：規則不是只寫給別人的。要求 Agent 遵守的規則，寫規則的人先遵守。
