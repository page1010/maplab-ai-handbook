...
## 實跑教訓（從廣告帳號截圖歸納，2026-07-03）

### PMax（最高成效）轉單差——多角化「不投廣告」已被印證

**教訓**：Google PMax 在 MAPLAB 外燴場景已實跑並**已停用**，原因是轉單成效不佳。

**根本原因**（補入矩陣）：

| 安索夫格子 | PMax 的問題 | 對矩陣的影響 |
|---|---|---|
| ②③ 市場/產品開發（冷溫受眾） | PMax 黑箱，無法做 message-match；外燴是「高考慮 × LINE 詢價」產品，黑箱優化方向跑偏 | 冷/溫層改用分受眾 Meta + 手動受眾包，不再用 PMax |
| ④ 多角化（新品新市場） | 新市場 PMax 更沒有歷史轉換資料，等於燒錢投石問路 | 強化「多角化不投廣告」決策，SEO 有機觸達優先的策略已被實跑驗證正確 |

**結論**：
- PMax 適合「黑箱有足夠轉換資料可優化」的場景（電商標準化商品）
- 外燴「高考慮 × LINE 詢價成交」不符合這個前提
- 重回「分受眾 × 分廣告 × 分 landing」三溫暖漏斗（詳見 `docs/ad-funnel-battle-plan.md`）
